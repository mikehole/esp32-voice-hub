...
"}"
"function resetWifi(){"
"if(confirm('Reset WiFi? Device will restart in setup mode (AP).')){"
"fetch('/api/wifi/reset').then(()=>{"
"alert('WiFi cleared. Connect to ESP32-Voice-Hub network to reconfigure.');"
"});"
"}"
"}"
...
"document.getElementById('wifiStatus').innerHTML='<div class=\"status info\">Connecting to '+ssid+'...</div>';"
"fetch('/api/wifi/connect',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({ssid:ssid,password:pass})});"
"setTimeout(function(){"
"document.getElementById('wifiStatus').innerHTML='<div class=\"status ok\">"
"<strong>WiFi saved!</strong><br><br>"
"The device is now connecting to your network.<br><br>"
"1. Reconnect your phone/laptop to <strong>'+ssid+'</strong><br>"
"2. Find the device at its new IP (check your router)<br>"
"3. Visit <strong>http://&lt;new-ip&gt;/admin</strong> to continue setup"
"</div>';"
"},2000);"
"}"
...
esp_err_t web_server_start(void)
{
    ...
    config.max_uri_handlers = 28;  // We have ~25 endpoints
